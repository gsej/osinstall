...
##############################################################################
################################# Version 1.5 ################################
######################## Valid ONLY for PiCoolFN HAT #########################
################ Please DO NOT user with any other hardware ##################
##############################################################################
#
# DESCRIPTION:
#               This script uploads firmware to PiCoolFAN HAT. Only mandatory input is new PiCoolFAN firmware.   
#
# RETURN CODES:
#               0 - Sucessfull update   
...
# VERSION:      1.5 adopted for PiCoolFAN August 2019 by PiModules
...
#                 v1.4 adopted for UPS PIco December 2014 by PiModules
#                 v1.5 adopted to PiCoolFAN. 
#
...
class FWUpdate(object):
  ...
  def power_detect(self):
    ...
    if not self.power and not self.power_warned:
      ...
      PIco= self.serial.Serial(port=self.port,baudrate=9600,stopbits=2,timeout=0.1,rtscts=0,xonxoff=True)
  ...
  def factory_reset(self):
    print "Invoking factory defaults of PIco..."
    time.sleep(5)
    status=False
    # Set up the connection to the PIco
    PIco = self.serial.Serial(port=self.port,baudrate=9600,stopbits=2,timeout=0.1,rtscts=0,xonxoff=True)
    ...
    time.sleep(5)
    i2c.write_byte_data(0x60, 0x08, 0xAA) 
     #close the channel to PIco
    PIco.close()
